...
package com.haha.fastproject.base.utils.notification.builder;
...
import com.haha.fastproject.base.utils.notification.NotificationUtils;
...
public class BaseBuilder {
    ...
    public void build() {
        ...
        mBuilder = new NotificationCompat.Builder(BaseApplication.getInstance(), mChannelId);

        if (mSmallIcon > 0) {
            mBuilder.setSmallIcon(mSmallIcon);// 设置顶部状态栏的小图标
        }
        if (mBigIcon > 0) {
            mBuilder.setLargeIcon(BitmapFactory.decodeResource(BaseApplication.getInstance().getResources(), mBigIcon));
        }
